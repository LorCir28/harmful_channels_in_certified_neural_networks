...
class BaseScheduler():
    ...
    def get_cur_eps(self, normalise=True):
        ...
        if normalise:
            ...
            if torch.is_tensor(self.cur_eps) and self.cur_eps.dim() == 3:
                std_tensor = std_tensor.view(-1, 1, 1)
            return torch.tensor(self.cur_eps) / std_tensor
        return self.cur_eps
    ...
    def get_max_eps(self, normalise=True):
        if normalise:
            std_tensor = torch.tensor(self.std)
            if torch.is_tensor(self.eps) and self.eps.dim() == 3:
                std_tensor = std_tensor.view(-1, 1, 1)
            return torch.tensor(self.eps) / std_tensor
        return self.eps
...
class SmoothedScheduler(BaseScheduler):
    ...
    def batch_step(self):
        if self.ramp_up == 0:
            self.cur_eps = self.eps
            self.cur_kappa = self.end_kappa
            self.cur_beta = self.end_beta
            self.no_batches += 1
            return
        ...
        linear_value = mid_value + (final_value - mid_value) * (step - mid_step) / (final_step - mid_step)
        if torch.is_tensor(linear_value):
            linear_value = torch.min(linear_value, final_value)
        else:
            linear_value = min(linear_value, final_value)
        ...
        if torch.is_tensor(self.cur_eps):
            self.cur_eps = torch.min(self.cur_eps, self.eps)
            if torch.is_tensor(self.cur_kappa):
                self.cur_kappa = torch.max(self.cur_kappa, torch.tensor(self.end_kappa, device=self.cur_kappa.device))
            else:
                self.cur_kappa = max(self.cur_kappa, self.end_kappa)
                
            if torch.is_tensor(self.cur_beta):
                self.cur_beta = torch.max(self.cur_beta, torch.tensor(self.end_beta, device=self.cur_beta.device))
            else:

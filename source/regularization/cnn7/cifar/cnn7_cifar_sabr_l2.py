...
def main():
    # Hyperparameters & Macros from CIFAR SABR training script
    BATCH_SIZE = 128
    EPSILON = 8 / 255
    NUM_EPOCHS = 10
    LR = 0.00003
    ...
    json_path = os.path.join(PROJECT_ROOT, 'jsons', 'cnn7', 'cifar', 'cnn7_cifar_sabr_scores.json')
    ...
    out_path = os.path.join(out_dir, 'cnn7_cifar_sabr_l2.pt')

    pretrained_path = os.path.join(PROJECT_ROOT, 'model_weights', 'cnn7', 'cifar', 'cnn7_cifar_sabr_8.pt')

    # 3. Evaluate baseline accuracies dynamically
    print("Evaluating baseline CNN7 SABR model on entire test set...")
    temp_model = CNN7_Shi(in_shape=in_shape, width=64, linear_size=512, n_classes=10).to(device)
    base_wrapper = SABRModelWrapper(
        model=temp_model, 
    ...
    # Define custom score-weighted L2 regularizer
    ...
    # Override regularizer in training package

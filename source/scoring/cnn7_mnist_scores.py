...
def compute_attribution_scores(model_path, loader, eps_std, epsilon, device):
    ...
    model = CNN7_Shi(in_shape=(1, 28, 28), width=64, linear_size=512, n_classes=10).to(device)
...
def main():
    ...
    weights_dir = os.path.join(PROJECT_ROOT, 'model_weights', 'cnn7', 'mnist')
    jsons_dir = os.path.join(PROJECT_ROOT, 'jsons', 'cnn7', 'mnist')
    os.makedirs(jsons_dir, exist_ok=True)

    models_to_eval = [
        ("IBP", os.path.join(weights_dir, 'cnn7_mnist_ibp_3.pt')),
        ("CROWN-IBP", os.path.join(weights_dir, 'cnn7_mnist_crown_ibp_3.pt')),
        ("SABR", os.path.join(weights_dir, 'cnn7_mnist_sabr_3.pt')),
        ("MTL-IBP", os.path.join(weights_dir, 'cnn7_mnist_mtl_ibp_3.pt'))
    ]
    ...
    for model_name, model_path in models_to_eval:
        ...
        json_name = f'cnn7_mnist_{model_name.lower().replace("-", "_")}_scores.json'

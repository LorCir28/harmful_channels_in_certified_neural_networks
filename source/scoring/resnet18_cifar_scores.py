...
def compute_attribution_scores(model_path, loader, eps_std, epsilon, device):
    ...
    # We expect 17 ReLU layers in ResNet18 (1 input relu + 4 layers * 2 blocks * 2 relus = 17)
    ...
    # Key: node_name, Value: List of tensors of shape [batch_size, num_channels]
        ...
        with torch.no_grad():
            for name in relu_node_names:
                ...
                if (hasattr(node, 'lower') and hasattr(node, 'upper') and 
                    ...
                    score_total = score_lower + score_upper # Shape: [B, C, H, W] or [B, C]
...
def main():
    ...
    weights_dir = os.path.join(PROJECT_ROOT, 'model_weights', 'resnet18', 'cifar')
    jsons_dir = os.path.join(PROJECT_ROOT, 'jsons', 'resnet18', 'cifar')
    os.makedirs(jsons_dir, exist_ok=True)

    models_to_eval = [
        ("IBP", os.path.join(weights_dir, 'resnet18_cifar_ibp_8.pt')),
        ("CROWN-IBP", os.path.join(weights_dir, 'resnet18_cifar_crown_ibp_8.pt')),
        ("SABR", os.path.join(weights_dir, 'resnet18_cifar_sabr_8.pt')),
        ("MTL-IBP", os.path.join(weights_dir, 'resnet18_cifar_mtl_ibp_8.pt'))
    ]
    ...
    for model_name, model_path in models_to_eval:
        ...
        json_path = os.path.join(jsons_dir, f'resnet18_cifar_{model_name.lower().replace("-", "_")}_scores.json')

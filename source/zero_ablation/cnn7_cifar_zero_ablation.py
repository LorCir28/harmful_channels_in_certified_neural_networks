...
def main():
    EPSILON = 8 / 255
    MODEL_WEIGHTS_PATH = os.path.join(PROJECT_ROOT, 'model_weights', 'cnn7', 'cifar', 'cnn7_cifar_mtl_ibp_8.pt')
    ...
    temp_model = CNN7_Shi(in_shape=(3, 32, 32), n_classes=10).to(device)
    if os.path.exists(MODEL_WEIGHTS_PATH):
        ...
        model = CNN7_Shi(in_shape=(3, 32, 32), n_classes=10).to(device)
    ...
    for l_idx, layer_name in enumerate(layers_ordered):
        ...
        # Extract the mask from the sequential block
        ...
        # Label with the architectural layer depth (6 down to 1)
        layer_labels.append(str(len(layers_ordered) - l_idx))

    # Save to JSON
    out_file = os.path.join(PROJECT_ROOT, 'jsons', 'cnn7', 'cifar', 'cnn7_cifar_mtl_ibp_harmful_channels.json')

...
L1_REG_WEIGHT = 1e-6
GRADIENT_CLIP = 10
SHI_REG_WEIGHT = 0.0 # Standard CROWN-IBP usually doesn't use SHI penalty
START_BETA = 1.0 # 1.0 = IBP dominant early on
END_BETA = 0.0   # 0.0 = CROWN-IBP dominant at the end
...
def main():
    ...
    print(f"Using {len(train_loader.dataset)} samples for CROWN-IBP training.")

    in_shape = [1, 28, 28]
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    
    out_dir = os.path.abspath(os.path.join(PROJECT_ROOT, 'model_weights', 'cnn7', 'mnist', 'cnn7_mnist_crown_ibp'))
    ...
    wrapped_model.eval()
    eval_results = wrapped_model.evaluate(test_loader, eval_method='CROWN-IBP')
    
    # CrownIBPModelWrapper.evaluate returns tuple of 3 if standard wrapper logic applies
    # (std_acc, cert_acc, adv_acc) but sometimes just 2 depending on version. We unpack safely.
    ...
    out_path = os.path.join(out_dir, 'cnn7_mnist_crown_ibp.pt')
    print(f"Saving final weights to {out_path}")
    torch.save(wrapped_model.state_dict(), out_path)
    
    # Also save to the main model_weights folder to maintain consistency with the rest of the scripts
    out_dir_main = os.path.join(PROJECT_ROOT, 'model_weights', 'cnn7', 'mnist')
    os.makedirs(out_dir_main, exist_ok=True)
    out_path_main = os.path.join(out_dir_main, 'cnn7_mnist_crown_ibp_3.pt')

...
def main():
    BATCH_SIZE = 128 # (da tutti i paper)
    EPSILON = 8 / 255
    NUM_EPOCHS = 160 # (da sabr)
    LR = 0.0005 # (da tutti i paper)
    WARM_UP_EPOCHS = 1 # (da sabr paper)
    RAMP_UP_EPOCHS = 80 # (da sabr paper)
    LR_DECAY_MILESTONES = (120, 140) # (da sabr paper)
    LR_DECAY_FACTOR = 0.2 # (da tutti i paper)
    L1_REG_WEIGHT = 0.0 # (da sabr paper, in mtl-ibp training non è specificato)
    GRADIENT_CLIP = 10 # (da tutti i paper)
    SHI_REG_WEIGHT = 0.5 # (da mtl-ibp paper), called lambda in the mtl-ibp paper
    ...
    print(f"Using {len(ibp_train_ds)} samples for IBP training.")
    ...
    out_path = os.path.join(out_dir, 'resnet18_ibp_2.pt')

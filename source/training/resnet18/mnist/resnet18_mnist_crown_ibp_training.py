...
def main():
    ...
    out_dir = os.path.abspath(os.path.join(PROJECT_ROOT, 'model_weights', 'resnet18', 'mnist', 'resnet18_mnist_crown_ibp'))
    ...
    wrapped_model.eval()
    clean_acc, cert_acc = wrapped_model.evaluate(test_loader, eval_method='CROWN-IBP')
    print(f"Final Clean Acc: {clean_acc * 100.0:.2f}%")
    print(f"Final Cert Acc: {cert_acc * 100.0:.2f}%")

    out_path = os.path.join(out_dir, 'resnet18_mnist_crown_ibp.pt')
